...
import TopTruyenFull from 'services/sources/TopTruyenFull'
...
async function getUrlPages (start, end) {
  ...
  const urls = []
  for (let i = start; i <= max; i++) { if (i > 0) urls.push(`https://toptruyenfull.com/truyen-moi-nhat.html?type=newest&page=${i}/`) }

  let data = []
  const urlPromise = urls.map(async (url) => {
    const urlPart = await TopTruyenFull.getInit(url)
    data = data.concat(urlPart)
  })

  await Promise.all(urlPromise)
  fs.writeFileSync('./toptruyenfull_urls' + start, JSON.stringify(data))
  return data
}
...
async function getStories (url) {
  const storyInfo = await TopTruyenFull.getDetail(url)
...
async function _processChapters (story, maxChapter) {
  ...
  for (let i = maxChapter; i > story.last_chapter; i--) {
    ...
    const chapter = await TopTruyenFull.getChapter(url)
...
async function init () {
  ...
  let subs = []
  urls = _.uniq(urls)
  const start = process.env.SKIP ? parseInt(process.env.SKIP) : 0
...
async function main () {
  await getUrlPages(1, 1000)
  await getUrlPages(1001, 1789)
  process.exit()
  const args = process.argv

  let act
  if (args.length === 0 || args[2] === 'init') { act = 'init' } else act = 'update'

  if (act === 'init') { await init() } else { await update() }
}
